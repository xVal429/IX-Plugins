AddCSLuaFile()
--[[
Edited by Scotnay#4780 to have optional 'voltage' settings.
]]
if (CLIENT) then
...
function SWEP:SecondaryAttack()
	if (self.Owner:KeyDown(IN_WALK)) and !self.Owner:GetEyeTrace().Entity:IsPlayer() then
		if self:GetVoltage() <= 2 then
			self:SetVoltage(self:GetVoltage() + 1)
		else
			self:SetVoltage(1)
		end
		if (IsFirstTimePredicted()) then
			if CLIENT then
				...
				LocalPlayer():Notify("You have set your stunstick to " .. thing[self:GetVoltage()] .. ".")
			end
		end
	end
