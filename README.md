...
> apollo-attributes ; A WIP rolling system which works similar to D&D. Includes the ability to do multiple dice rolls, crit fails, crit successes, etc. 1d20? Sure. 5d37? Yeah, you can do that. Also includes some unused stuff like when an attribute is used, and also contains a used section for aliases, so the player won't have to type in "deception" for instance, they can just type "dec". The code would probably make a medieval peasant explode, and it's not the most efficient, but it works.
...
> ix_stunstick ; Just an improved stunstick SWEP over the original.  Has voltage settings for varying damage. Not made by me.
